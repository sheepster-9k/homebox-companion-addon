...
if [ ! -f "$CONFIG_PATH" ]; then
    echo "ERROR: $CONFIG_PATH not found — add-on options not yet initialized" >&2
    exit 1
fi
...
export HBC_LLM_ALLOW_UNSAFE_MODELS="$(jq -r '.llm_allow_unsafe_models // false' "$CONFIG_PATH")"
...
if [ -z "${HBC_HOMEBOX_URL}" ]; then
    echo "ERROR: homebox_url must be configured in the add-on settings" >&2
    exit 1
fi
...
echo "  LLM Model:   ${HBC_LLM_MODEL:-not set}"
